...
- 180일 알림은 마지막 수정일, 생성일, 최근 방문일, 방문 기록 입력일 및 관계 단계 기록일 중 가장 최근 날짜로 계산합니다. 숫자형 타임스탬프와 날짜 문자열을 모두 처리합니다.
- 카드의 D+N은 실제 마지막 방문일만 기준으로 하며 한국 날짜의 자정마다 증가합니다. 오늘 방문은 D+0, 방문 날짜가 없으면 '방문일 미등록'입니다. 정보 수정이나 방문 기록 입력 시각은 카운트를 초기화하지 않습니다.
