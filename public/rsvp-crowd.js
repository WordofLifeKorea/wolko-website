// 실제 RSVP 참석 인원(totalGuests)만큼 미니어처 사람을 표지 안에 채워 넣는다.
// 손 흔들기·불꽃놀이 구경·걷기 역할은 특정 인물에게 고정하지 않고, 몇 초마다
// 무작위로 다른 사람에게 옮겨가며 계속 바뀐다.
...
export const MAX_VISIBLE = 14;
const ROLES = ['wave', 'lookup', 'walk'];
const ROTATE_MIN_MS = 3200;
const ROTATE_JITTER_MS = 2200;

export function personMarkup(palette, index) {
  const delay = (-(index % 5) * 0.13).toFixed(2);
  return `<span class="inv-person" style="animation-delay:${delay}s">` +
...
// 사람 수보다 역할 종류가 적으면 있는 만큼만 배정한다(1명이면 손 흔들기만, 2명이면 +불꽃놀이 구경).
...
function setRole(personEl, role) {
  personEl.classList.remove('is-lookup', 'is-walk');
  ...
  if (role === 'wave') {
    ...
    personEl.classList.add('is-walk');
  }
}
  ...
  async function refresh() {
    ...
    try {
      ...
      status.textContent = total
        ? `지금 ${total.toLocaleString('ko-KR')}명이 함께하기로 했어요`
        : '가장 먼저 함께해 주세요';
